...
def display_expense_dashboard(credit_df):
...
def show_dashboards(df):
    ...
    top_expenses = df.copy()

    # Agrupa por título e soma os valores
    top_expenses = top_expenses.groupby("title", as_index=False)["amount"].sum()

    # Cria uma coluna com o valor absoluto (para pegar maiores despesas)
    ...
    # Cria o gráfico
    fig = plt.figure()
    plt.barh(top_expenses["title"], top_expenses["abs_amount"])
    plt.title("Top 10 maiores despesas (por título)")
    plt.xlabel("Valor")
    plt.ylabel("Título")
    plt.tight_layout()
...
def main():
    ...
    tab_summary, tab_dash, tab_categ, tab_upload = st.tabs(["Summary", "Dashboard", "Categories", "Upload Data"])
    with tab_upload:
        handle_file_upload()
        if st.button("🔄 Buscar novas faturas no Gmail e enviar ao Drive"):
            with st.spinner("Buscando faturas e sincronizando..."):
